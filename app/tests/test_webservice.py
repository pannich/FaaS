# test_webservice.py
# Original source: https://github.com/mpcs-52040/MPCS-FaaS-Tests/blob/main/tests/test_webservice.py
...
def test_roundtrip_result():
    ...
    resp = requests.get(f"{base_url}result/{task_id}")
    while resp.json()['status'] not in ["COMPLETED", "FAILED"]:
        resp = requests.get(f"{base_url}result/{task_id}")
        ...
        if resp.json()['status'] in ["COMPLETED", "FAILED"]:
            ...
            logging.warning(s_result)
            result = deserialize(s_result['result'])
            print(f"received: {result}, expected: {number*2}" )
            assert result == number*2
            break
        time.sleep(0.01)
